...
//     for (let index = 0; index < mapJsonssssssss[props.adcode].features.length; index++) {
//         const item = mapJsonssssssss[props.adcode].features[index];
//         options[item.properties.adcode] = item.properties;
...
const setOption = function () {
	nextTick(() => {
		...
		const viewControl = {
			// x轴旋转角度
			alpha: props.sets.alpha || mapOptions[adcode.value].alpha || 50,
			// y轴旋转角度
			beta: props.sets.beta || mapOptions[adcode.value].beta || 0,
			// 中心点位置
			center: props.sets.center || mapOptions[adcode.value].offset || [0, 0, 0],
			// 视距控制
			distance: props.sets.distance || mapOptions[adcode.value].distance || 105,
			// 是否开启自动旋转
					...
					data: (() => {
						...
						for (let index = 0; index < mapOptions[adcode.value].regions.length; index++) {
							...
							res.push({
								...
								itemStyle: {
									color: mapOptions[item].regionColor,
								},
							});
						}
						return res;
					...
					data: (() => {
						...
						for (let index = 0; index < mapOptions[adcode.value].regions.length; index++) {
							...
							res.push({
								name: mapOptions[item].name,
								lineStyle: {
									color: mapOptions[item].linesColor,
								},
								coords: [[...mapOptions[item].symbol, 0], adcode.value === mapOptions[item].adcode ? [...mapOptions[item].symbol, 0] : [...mapOptions[item].label, 0.6]],
							});
						}
						return res;
					})(),
				},
				{
					...
					data: (() => {
						...
						for (let index = 0; index < mapOptions[adcode.value].regions.length; index++) {
							...
							res.push({
								name: mapOptions[item].name,
								value: [...mapOptions[item].symbol, 0],
							});
						}
						return res;
					})(),
				},
				{
					...
					data: (() => {
						...
						for (let index = 0; index < mapOptions[adcode.value].regions.length; index++) {
							...
							res.push({
								name: mapOptions[item].name,
								value: adcode.value === mapOptions[item].adcode ? [...mapOptions[item].symbol, 0] : [...mapOptions[item].label, 0],
							});
						}
						return res;
					})(),
