...
<template>
    <el-radio-group :size="sets.size" :disabled="sets.disabled" :text-color="sets.textColor" :fill="sets.fill" class="base-radio">
        <template v-if="sets.type == 'button'">
            <base-badge v-for="item in options" :key="item.value" v-model="item.badge" :sets="item.sets">
                <template #default>
        ...
        <template v-else>
            <el-radio v-for="item in options" :key="item.value" :value="item.value" :disabled="item.disabled" :border="sets.border">
                {{ item.label }}
            </el-radio>
        </template>
    </el-radio-group>
