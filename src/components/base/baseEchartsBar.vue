...
onUnmounted(() => {
    if (bar) bar.getZr().off("click");
    bar = null;
    window.removeEventListener("resize", resizeEv);
});
const props = withDefaults(
    defineProps<{
        modelValue: TsEchartsBar.Model;
        options: TsEchartsBar.Options;
        sets?: TsEchartsBar.Sets;
    }>(),
    {
        sets: () => {
            return {};
...
const barRef = ref();
let bar: TsEcharts.ECharts | null;
...
const processMarkLine = function (index: number, markLine?: TsEchartsBar.MarkLine) {
...
const init = function () {
    nextTick(() => {
        ...
        bar.getZr().on("click", function (params) {
            let pointInPixel = [params.offsetX, params.offsetY];
            if (bar!.containPixel('grid', pointInPixel)) {
                let pointInGrid = bar!.convertFromPixel({
                    seriesIndex: 0
                }, pointInPixel);
                let xIndex = pointInGrid[0]; //索引
                let handleIndex = Number(xIndex);
                let seriesObj = bar!.getOption() as any; //图表object对象
...
const resize = function () {
    nextTick(() => {
        if (!bar || !barRef.value.offsetWidth) return;
        bar.resize();
    });
};
...
<template>
    <div class="base-echarts-bar" ref="barRef"></div>
</template>
<style scoped>
.base-echarts-bar {
    width: 100%;
