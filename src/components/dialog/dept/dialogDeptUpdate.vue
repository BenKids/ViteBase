...
//api 表单提交
const {send: sendSubmit, form, reset, updateForm} = apiDept.deptUpdate();
//cpa 弹框表单组合式api
...
<template>
    <base-dialog v-model="visible" title="修改部门" :sets="setsDialog" @before-close="onCancel">
        <base-form v-model="form" ref="formRef" :sets="setsForm">
            <base-form-tree-select label="上级部门" v-model="form.parentId" prop="parentId" :options="optionsDeptId" :sets="setsDeptId" v-if="form.parentId != 0"></base-form-tree-select>
